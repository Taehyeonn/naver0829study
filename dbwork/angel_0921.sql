...
--단점 : 조회비율이 매우 높은 시스템의 경우에는 테이블간 join 연산이 반복적으로 
--이뤄지기 때문에 질의 응답 속도가 살짝 늦어질수있다.

--sudent의 num을 외부키로 갖는 새로운 테이블 stuinfo 를 만들어보자
--외부키는 테이블 설정시에 설정해도 되고 나중에 설정해도 됨
...
--student의 num을 외부키로 설정(이때 주의: student의 num은 반드시 primary key 여야 한다)
--ppt 29p
alter table stuinfo add CONSTRAINT stuinfo_fk_num FOREIGN key(num) references student(num); --sudent를 참조하는 

--stuinfo에 데이터르 추가해보자
--오류메세지 : 무결성
-- student 테이블이 부모테이블 : 거기에 num8이 없어서 오류가 나는것
insert into stuinfo VALUES (seq_stu.nextval, 8,'강남','010-0000-0000');


insert into stuinfo VALUES (seq_stu.nextval, 9,'서초','010-1234-0000');
insert into stuinfo VALUES (seq_stu.nextval, 10,'송파','010-4321-0000');
...
-- 컬럼명이 중복되지 않는다면 테이블명은 생략해도 된다 (자바의 this 같은 느낌)
-- (위랑 같은 코드)
select 
...
--sub 테이블에 연결된 데이터가 있늗네 부모테이블(student)의 해당 데이터를 삭제하고 싶을 경우
--ORA-02292: 무결성 제약조건(ANGEL.STUINFO_FK_NUM)이 위배되었습니다- 자식 레코드가 발견되었습니다
delete from student where num=10; --오류

--삭제하려면 자식테이블(stuinfo)의 데이터를 먼저 삭제후 부모테이블(student)의 데이터 삭제
delete from stuinfo where num=3; --자식테이블 먼저 삭제
delete from student where num=3; --부모테이블 먼저 삭제

--부모테이블 drop
--오류발생 : 왜래키에 참조되는 고유/기본키가 테이블에 있습니다
drop table student;
